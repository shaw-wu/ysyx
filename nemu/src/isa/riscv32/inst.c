...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
	...
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai	 , I, R(rd) = (int32_t)src1 >> (imm & 0x0000001f));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall	 , N, s->dnpc = isa_raise_intr(0x0000000b, s->snpc), etrace(s->pc, 3)); 
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = CSR(MEPC), etrace(s->pc, 4)); 
