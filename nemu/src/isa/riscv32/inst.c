...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, JMP((src1 + imm) & 0xfffffffe), printf("dnpc:0x%08x, 0x%08x, src1:0x%08x, imm:0x%08x\n", s->dnpc, (src1+imm) & 0xfffffffe, src1, imm), ftrace_jalr(s->pc, s->dnpc, rd, rs1));
